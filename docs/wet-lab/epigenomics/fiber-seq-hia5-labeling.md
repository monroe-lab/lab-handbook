...
**Purpose:** Treat intact nuclei with the Hia5 m6A methyltransferase so that accessible DNA
is methylated and protein-bound DNA is left unmethylated. This ten-minute reaction *is* the
Fiber-seq signal. Everything before it is preparation and everything after it is readout.
...
Epicypher CUTANA Fiber-seq product documentation. Background on what the reaction measures
is in [[fiber-seq-master-protocol]].

## Background

Hia5 is a bacterial non-sequence-specific N6-adenine methyltransferase. It methylates any
adenine it can physically reach, so the readout is **accessibility, not a sequence motif**.
SAM is the methyl donor and is the labile reagent in the system.

Three things about this step drive everything:

- **The reaction is short on purpose.** Ten minutes at 25 °C. Longer incubation pushes toward
  over-labeling, which erodes footprint resolution — the check for it is inferred nucleosome
  length shorter than the real ~150 bp.
- **It follows Michaelis-Menten kinetics**, scaling with substrate. That is the logic behind
  Epicypher's genome-size scaling recommendation, which the lab tested and rejected for plants
  (see the Variation section).
- **Wide-bore tips throughout.** You are handling nuclei that are about to yield the HMW DNA a
  Revio run depends on.

## Time estimate

5 min pellet spin plus ~10 min setup, then a **10 min reaction**. Buffer can be made ahead;
**SAM is added at the activation step, not stored in the buffer.**

## Required input

Counted nuclei from [[fiber-seq-nuclei-isolation]], aliquoted to the target number and
pelleted at 1,400 × g for 5 min, ready to resuspend in activation buffer.
...
| [[spermidine]] | 0.5 mM | 2 M | 12.5 µL | 2.5 µL |
| BSA | 0.1% | — | 50 mg | 10 mg |
...
> **Critical — sucrose.** Both plant papers include **400 mM sucrose** in the working buffer,
...
The numbered steps below are the lab's 100 µL reaction. For other scales see the Variation
section.
...
> 0.8 mM.
> `[VERIFY: confirm with Vianney that the 2.5 µL was a deliberate correction rather than a
> coincidence — it matters for how much else in the papers should be trusted.]`
...
The 06.18.2026 variant keeps the lab's 100 µL scale but raises enzyme to 2 µL, i.e. Epicypher's
enzyme amount at the plant papers' volume. `[VERIFY: what was the 06.18.2026 test asking, and
was it scored?]`
...
`[VERIFY: are the gels from these timecourses interpretable? If so, score them — this is the
data that would justify or change the 10 min default.]`
