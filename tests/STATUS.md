...
| R18 | — | Feedback round 2026-04-22 (#137-#142, tutorials): editor-modal body area now auto-grows with `height:'auto'` + `minHeight:'500px'` so edit mode doesn't collapse on short bodies (#140, #142); lab-map tree defaults to fully collapsed via `initialDepth:0` (#139); inventory toolbar slimmed, Add-Item promoted to stat-card-shaped button matching sample-tracker, `td`/`th` padding tightened (#138); sample-tracker gained a larger markdown-flavored description field, `[[` autocomplete on notes, and a "Physical instances" section listing tubes/boxes referencing the sample (#137); calendar New-Event modal wires `[[` autocomplete on Member(s) and Notes, Notes renamed "Body (markdown)" with 180px min-height (#141); `Lab.modal.form` fields gain `wikilinkAutocomplete` + `minHeight` options, `Lab.wikilinkAutocomplete.attachTextInput` added for plain inputs/textareas; tutorials.html received `lab.js` (FAB) and layout bug fixed (extra `</div>`s closing grid early); `scripts/check-fab.py` added to deploy.yml to prevent pages shipping without the 💡 FAB |
| R19 | — | **Sample → Accession schema overhaul** (2026-04-22). Two-tier concept/instance model for biological samples, mirroring reagents↔bottles: `accession` is the concept (tree, genotype, MA line) and carries the pipeline status; instances attach via `of: [[accessions/<slug>]]` as one of `sample` (🌿 leaf/tissue collection), `extraction` (🧪 DNA/RNA), `library` (📖 sequencing prep), or `pool` (🔀 multiplex). All 228 rows in `samples.json` migrated to individual `docs/accessions/*.md` files via `scripts/migrate-samples-to-accessions.py`; `docs/samples/` retired (2 files moved to `docs/accessions/`, 2 test fixtures deleted). All existing `[[samples/*]]` wikilinks rewritten across the vault. New `app/accessions.html` tracker reads from the object-index (one row per accession), inline editable status pill, filters by project/status/seq-type/lead, instance-count badge next to each name. Old `/sample-tracker/` URL becomes a redirect stub. Nav renamed "Samples" → "Accessions" (fingerprint icon). `_addInstance` in editor-modal prompts for kind (sample/extraction/library/pool) when the concept is an accession. `renderBacklinksPane` now classifies sample/extraction/library/pool/tube as "instances in lab" regardless of whether they link via `of:` or body wikilinks. `build-object-index.py` hardened against PyYAML's implicit date parsing (coerces `datetime.date` → ISO string on JSON dump) |
| R19b | — | **Follow-up round** (2026-04-22): `project` field was missing from EXTRACT_KEYS/INDEX_KEYS so the accessions tracker showed a blank Project column; added to both and rebuilt. #143: daily notebook template adds empty-paragraph spacers between sections and a starter Results table; wikilink `[[` autocomplete sanitizes post-insertion markdown to guarantee a blank line before every ATX heading so the next `## Heading` isn't absorbed by the paragraph above. #144: inventory Add-Item rewritten as a two-step picker (new bottle of existing chemical vs new chemical concept); step 2a searches existing concepts and hands off to `Lab.editorModal._addInstance`; step 2b retains the concept-creation form. #145: nav TABS reordered to Grey's 10-tab spec, Protocols+Waste moved to a new always-overflow `SECONDARY_TABS` accessible via a desktop "+" button that mirrors the mobile bottom-bar popover; tab row replaced `overflow-x:auto` with `overflow:hidden` + dynamic `updateOverflow()` that hides trailing tabs when the viewport narrows and surfaces them in the "+" popover, skipping the active tab. #146: `.doc-type-filter` uncapped so every type pill is visible instead of trapped behind a nested 110px scroll. Also renamed `docs/lab-management/sample-tracker-guide.md` → `accession-tracker-guide.md` with full rewrite covering the concept/instance vocabulary and new URL. Smoke test `tests/r19b-smoke.mjs` (6/6 pass) covers nav order, overflow popover, project column, inventory add picker, bottle-picker results, and pill uncap |
| R20 | — | **Accession schema overhaul** (2026-04-22, #155-#161). Four broad pipeline status buckets (`active` / `waiting` / `completed` / `archived`) replace the old 14-state pipeline, with the old value preserved verbatim in `status_note`. Priority is now 0-3 stars (integer); emoji priorities (⭐ / 💎 / 🌾) auto-migrate via `scripts/migrate-accession-schema.py`. `lead` → `people` (wikilink-taggable multi-person), `current_blocker` → `status_note` (free-text), species made optional, sequencing_type demoted to non-primary tag. Biological stock types (`seed` / `plasmid` / `glycerol_stock` / `agro_strain` / `dna_prep`) marked deprecated aliases in the `accessions` group so legacy files keep rendering; new objects default to `accession`. Four files moved from `docs/stocks/` → `docs/accessions/` (col-0-wild-type, bl21-de3-competent-cells, two redwood DNA preps) with `legacy_stock_type` preserved in frontmatter. Chemical re-triage bundled in: `sucrose` reclassified `chemical` → `reagent`; `hydrochloric-acid` + `liquid-nitrogen` stay `chemical` (both warrant EHS handling). `rnase-aliquot` concept renamed `rnase` and wikilinks across the vault rewritten |
| R20b | — | **Feedback round** (2026-04-22, #162-#167). #162: pin-favorite pill on the People dashboard no longer collapses every `<details>` section — `togglePinFavorite()` captures open-section set + scroll position before the render and restores them after. #163: nav reorder to Grey's 12-tab spec (Tutorials, Notebooks, Protocols, Accessions, Inventory, Waste, Projects, People, Calendar, Lab Map, Apps, Wiki); `SECONDARY_TABS` emptied so the "+" More button only appears when `updateOverflow()` detects a primary tab actually doesn't fit; `BOTTOM_TABS` (mobile) rebalanced to Wiki/Notebooks/Accessions/Inventory. #164: stale `docs/notebooks/test-user/` + a phantom `2026-04-08-test.md` purged (local index patch leftover that had no file on disk). #165: `of:` datalist in the editor now offers `accessions` when the current type is `sample` / `extraction` / `library` / `pool` / `tube` — previously restricted to `samples` / `resources` / `stocks` which meant the canonical `of: [[accessions/...]]` shape never appeared in the picker. #167: inventory Add step 2 wording broadened from "A new chemical we haven't had before" → "A new reagent, chemical, kit, equipment, buffer, or solution" — most new items aren't chemicals and the old phrasing misled users |
| R21 | — | **Chemical type cleanup + storage status + framework banners** (2026-04-22, #168, #169). 157 files in `docs/resources/` retyped. Pass 1 (`scripts/reclassify-ehs-chemicals.py`): cross-references `docs/lab-safety/chemical-inventory.md` (UC Davis EHS export, 196 containers across Chemical / Corrosive / Flammable / Refrigerator cabinets) and flips 128 `reagent` → `chemical`, populating `cas:` and `location:` frontmatter from the EHS tables. Pass 2 (`scripts/reclassify-non-ehs-resources.py`): promotes 8 physically-present-but-EHS-unregistered pure chemicals to `chemical` (sucrose, DTT, Tween-20, DMF anhydrous, IPTG, PVPP, MS basal salt, 30% H₂O₂); flips 9 enzymes (benzonase, DNase, proteinase K, Q5 polymerase, tagmentase, Illumina TDE1, tn5-unloaded, rnase, thermo-dnase-i) to `enzyme`; flips 12 commercial kits/ready-mixes/ladders/stains (AMPure XP, Bio-Rad Laemmli / running buffer / protein standards, Bradford reagent, BSA, Cytiva speedbead, DNA ladder 1kb+, lambda DNA, KAPA HiFi, Roche protease inhibitor, SYBR Gold) to `kit`. `app/js/types.js` chemical pill recolored from teal `#009688` (same as reagent) to purple `#6a1b9a` so EHS-registered chemicals are visually distinct; icon was already `fa-atom`. Bulletin item added for [[Kehan Zhao]] listing the 8 unregistered chemicals with the EHS portal link. #168: 5th accession status bucket `storage` (blue pill, `inventory_2` icon) added between Waiting and Completed for seeds/cell lines/stocks in long-term stasis — `types.js` options, filter dropdown, stat card, STATUS_COLORS/LABELS/ALL_STATUSES all updated. #169: concept/instance orientation banners added to the top of Accessions (purple border, accessions color) and Inventory (teal border, inventory color) pages — small callouts explaining that the pages list general *concepts* (accessions = tree/genotype, inventory = chemical type) with physical *instances* (🌿 samples / 🧪 extractions / 📖 libraries / 🔀 pools · 🍶 bottles) linking out to Lab Map for physical navigation. Stale `docs/resources/ethyl-alcohol-absolute.md` duplicate deleted; one remaining `[[ethyl-alcohol-absolute]]` wikilink in `flammable-liquids-sop.md` retargeted at `ethanol-absolute`, and the other plain-text chemical names on that line upgraded to wikilinks for consistency. Final type distribution: **138 chemical, 27 kit, 9 enzyme, 9 reagent** (from 162 reagent / 2 chemical pre-cleanup). Remaining 9 reagents are oligos (IDT tn5me primers), prepared dilutions (ethanol-70), nuclease-free water, pH calibration buffers, a plasmid card, and two agar/TC-agar records |
| R22 | 9/9 | ✅ **Full-text ranked search** (2026-08-20, #188). `build-object-index.py` gains `build_search_index()` → `docs/search-index.json` (every .md under docs/, frontmatter + base64 data-URIs + image/link URLs stripped, 60 KB/doc cap, ~2 MB total, gitignored — regenerated at deploy). New `app/js/search.js` (`Lab.search.query(q, {paths, types, limit})`): lazy single-fetch of the index, multi-word AND, ranked scoring (exact/prefix/substring title ≫ heading ≫ body term frequency, phrase bonus), highlighted `<mark>` snippets. Wired into protocols.html (ranked flat results with snippets replace the category tree while searching; instant title-filter fallback until the index loads) and wiki.html (same upgrade inside `renderSidebar`, respects type chips). Unit test `tests/search-index.test.mjs` (vm sandbox against the real generated index): 9/9. Live-verified via Playwright on both pages |
| R23 | 7/7 | ✅ **Feedback fixes round** (2026-08-20, #178 #179 #182 #184). #184: `getMarkdownClean` converts Toast UI's standalone `<br>` lines (its serialization of empty WYSIWYG paragraphs) into ` ` spacer paragraphs — the daily template's #143 idiom — before the `\n{3,}` collapse, so Enter-created gaps survive the save round-trip; inline/table `<br>` unaffected (line must contain only the tag). #178: annotation src-swap tries relativeSrc/oldSrc/encoded/decoded variants and toasts an explicit error instead of silently no-oping; fires `containerEl._onchange` (notebooks draft autosave) after the swap; annotate.js toast now says the entry still needs Save. #179+#182: `renameEntry()` in notebooks.html — Rename button in both view-header render sites opens a Lab.modal.form with Title + Folder; title edits rewrite frontmatter + matching H1 in place (sha-checked save), folder moves create-at-new-path + delete-old with clash guard, object-index patches, `loadTree()` reload; slug preserved on moves. E2E smoke `tests/feedback-fixes-smoke.mjs` (live site, scratch entry in barb-m, auto-cleanup): 7/7 pass |
| R24 | 7/7 | ✅ **Text color in the editor** (2026-08-20, #187). Toast UI `color-syntax` plugin (+ its `tui-color-picker` dependency) loaded in `loadToast()` — best-effort, CDN failure degrades to the old no-color editor — and registered via `editorPlugins()` in all three `new toastui.Editor` sites. Toolbar gains the plugin's color-picker button automatically (desktop); registered even on mobile (empty toolbar) so the document model understands `<span style="color:...">` and mobile edits no longer strip existing colors. View-mode renderer already passed spans through (QA-34 sanitizer only strips script/iframe/on* attrs). E2E `tests/color-syntax-e2e.mjs` (live site, scratch barb-m entry, auto-cleanup): plugin load, toolbar button, exec color command, span in saved markdown, red render in view mode, span survives untouched edit/save cycle — 7/7. Highlighting (`<mark>`) NOT covered — plugin does foreground color only; `<mark>` still renders in view mode but gets stripped by editor round-trip |
| R25 | 5/5 | ✅ **Kanban task board** (2026-08-20). New `task` object type (`docs/tasks/`, fields: status todo/in_progress/done, people, project, due) + `tasks` group in `types.js` (exempt from concept-title uniqueness — tasks recur); `tasks` added to OBJECT_DIRS and `due` to EXTRACT_KEYS. New `app/board.html`: 3-column board rendered from the object index, HTML5 drag-and-drop between columns patches `status:` frontmatter via Contents API (optimistic render + revert-on-error, `saving` guard blocks drags while a card's create commit is in flight), per-column quick-add (Enter → new markdown file, slug from title, `.md` included in the index path — fresh cards 404'd without it), card click opens editor modal, title/person/project filter, overdue due-date highlighting. Nav gains a Board tab (view_kanban). Smoke `tests/board-smoke.mjs` (live site, auto-cleanup): columns, welcome card, nav tab, quick-add persists to repo, drag persists frontmatter — 5/5 |
| R26 | 5/5 | ✅ **"Ask the handbook" chat assistant** (2026-08-20). New `app/js/ask.js` loaded via lab.js on every page: floating chat FAB (teal, stacked above the issue reporter) opens a panel where lab members ask natural-language questions ("where is the agarose?"). Architecture: retrieval is deterministic and local — `Lab.search.query` ranks pages, `object-index` supplies frontmatter facts, `Lab.hierarchy.parentChain` supplies physical location chains — and the assembled SOURCE blocks ground a single **Google Gemini Flash free-tier** call (no paid API; each member pastes a free key from aistudio.google.com/apikey once, stored in localStorage like the GitHub PAT; setup picks a served flash model from ListModels so name drift doesn't break it). Answers cite `[[slug]]` tokens rendered as the standard clickable object pills via `Lab.wikilinks.processRendered`. Session-only chat history, 400/403 → re-key prompt. Smoke `tests/ask-smoke.mjs` (Gemini mocked via route interception): FAB, retrieval grounding, panel flow, pill rendering, key settings — 5/5 |
...
**Feedback backlog triage (2026-08-20):** all nine Jun–Jul feedback issues resolved except #187. Closed as already-fixed with explanations: #181 (local draft autosave, b0e8a752), #183 (deploy cancel-in-progress, fc12c3c6, zero failures since 07-02), #185 (Blobs API fallback eb807220 — **and Percival's wiped 2026-07-06 entry restored from git history**, commit 5689b076), #180 (right-click table context menu already existed; discoverability noted). Fixed in R23 and closed: #184 (blank gaps), #178 (annotation swap), #179+#182 (rename/move). #187 (text color) initially deferred, then shipped same day as R24 — the color-syntax plugin round-trip turned out clean; highlighting (`<mark>`) remains unsupported in the editor. Stale remote branch `claude/github-notebook-entry-deletion-gpwdfz` (fully merged) deleted.
...
20. **Editing a >1 MB notebook entry wiped all previously saved writing** — Re-opening a notebook entry for editing loaded a **blank editor**, and the next save then overwrote the real entry with only what was newly typed, silently deleting everything that was there. Root cause: `fetchFile()` in `github-api.js` reads via GitHub's **Contents API**, which only inlines base64 for blobs up to **1 MB** — for files 1–100 MB it returns `encoding: "none"` and an **empty** `content` string. Notebook entries with pasted screenshots (stored inline as base64 `data:` URIs) routinely exceed 1 MB, so their content decoded to `""` on load. The length-based save guards (issue #185) couldn't catch it: they were disabled because `originalContentLen` was 0 from the blank load, and even otherwise a freshly-pasted screenshot is as large as the old content, so the byte count never shrank. This is the real root cause behind @percival-singson's reports (`2026-07-06.md` grew to 2.1 MB, then on the next save had all its syri/minimap2 notes replaced by two screenshots). **Fixed:** `fetchFile()` now falls back to the **Blobs API** (base64 up to 100 MB) using the blob sha the Contents API still returns when it omits the content — so large entries load their full content, which also re-arms the save guards as a side effect. Regression coverage in `tests/large-file-fetch.test.mjs` (exercises the real `fetchFile` via a vm sandbox + mocked fetch: small file loads directly from Contents, >1 MB file recovered byte-exact via the blob fallback). Related earlier reports: #185 (one-time wipe — the empty-content save guard was a mitigation, not the root fix), #181 (local draft autosave, a safety net for the same class of loss). (Issue #185)
