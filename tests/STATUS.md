...
- [ ] **Lab map: add-item UI + hierarchical location objects** — No UI to add places/items to the lab map. Needs a well-designed hierarchy up front so we don't back-correct weak design: room → freezer/fridge/shelf → box → tube. Each level is a markdown object with its own type. Standard 100-tube boxes should render with a tube-grid view; tubes are special (can have contents, side label, top label). Get the object-type system right before adding UI. (Issue #18)
- [ ] **Lab map: deprioritize visual map, prioritize hierarchy** — For now replace the clickable floor plan with a static screenshot + "map design in progress, use list of places below" overlay. Focus build-out on the hierarchical structure of places/objects underneath. Visual map comes later. (Issue #19)
- [ ] **Frictionless location tagging from notebooks** — Make it trivial for lab members to say "I put this here" while writing a notebook entry. Creating a new DNA sample in a notebook should spawn a markdown object (e.g. "pistachio DNA extraction") that's stored in a specific freezer box with a position. Likely needs (a) optional location frontmatter on inventory/sample items, (b) a hierarchical breadcrumb in the item card view, and (c) a way to edit frontmatter/meta fields from the wiki edit view (currently unclear whether that's possible). Workflow needs thorough end-to-end testing. (Issue #20)
